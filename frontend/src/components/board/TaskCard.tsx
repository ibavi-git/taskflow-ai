...
export function TaskCard({ task, isOverlay = false }: TaskCardProps) {
  ...
  return (
    <div
      ...
      {...listeners}
      className={`
        bg-card rounded-lg border border-border shadow-sm overflow-hidden group hover:border-primary/50 transition-colors
        ...
        <div className="p-3 cursor-grab active:cursor-grabbing block">
          ...
          <div className="flex items-center justify-between mt-3">
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              ...
              {/* Date */}
              {task.dueDate && (
                <div className={`flex items-center gap-1 ${isOverdue ? 'text-destructive font-medium' : isDueToday ? 'text-orange-500 font-medium' : ''}`}>
