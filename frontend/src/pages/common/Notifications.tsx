...
import { useDeleteNotification, useListNotifications, useMarkAllNotificationsRead, useMarkNotificationRead, type Notification } from "@workspace/api-client-react";
        ...
        <div className="divide-y divide-border rounded-lg border border-border bg-card">
          {notifications.length > 0 ? (
            notifications.map((notification: Notification) => (
              <article key={notification.id} className="flex items-start justify-between gap-4 p-5">
                ...
                <div className="flex items-center gap-2">
                  {!notification.read ? (
                    <Button type="button" variant="outline" size="sm" onClick={() => markOneRead.mutate({ notificationId: notification.id })}>
                      Mark read
                    </Button>
                  ) : null}
