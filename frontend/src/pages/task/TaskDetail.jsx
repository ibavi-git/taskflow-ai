...
function TaskDetail({ params }) {
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="space-y-4">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {task.labels?.map((label) => <Badge key={label.id} variant="outline" style={{ borderColor: label.color, color: label.color }}>
                  {label.name}
                </Badge>)}
            </div>
          ...
          <Tabs defaultValue="details" className="w-full">
            <TabsList>
              ...
              <TabsTrigger value="comments">Comments {(task.commentCount ?? 0) > 0 && `(${task.commentCount})`}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="details" className="mt-6 space-y-6">
              <div className="bg-card border border-border rounded-xl overflow-hidden">
                <div className="p-4 border-b border-border flex items-center justify-between bg-muted/30">
                  ...
                  {!isEditingDesc ? <Button variant="ghost" size="sm" onClick={() => setIsEditingDesc(true)}>Edit</Button> : <div className="flex gap-2">
  ...
  /> : <div className="whitespace-pre-wrap text-muted-foreground">
            ...
            <TabsContent value="comments" className="mt-6 space-y-6">
              <div className="space-y-4">
                {commentsLoading ? [1, 2].map((i) => <Skeleton key={i} className="h-24 w-full" />) : comments && comments.length > 0 ? comments.map((comment) => <div key={comment.id} className="flex gap-4">
                      <Avatar className="h-8 w-8">
                        <AvatarImage src={comment.author.avatar || void 0} />
                    ...
                    </div>) : <div className="text-center py-8 text-muted-foreground text-sm border border-dashed border-border rounded-lg">
                    No comments yet.
                  </div>}
              </div>
