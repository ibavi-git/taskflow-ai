  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => (
          <article key={card.key} className="rounded-lg border border-border bg-card p-5">
          ...
          <div className="divide-y divide-border">
            {upcoming.length > 0 ? (
              upcoming.slice(0, 8).map((task) => (
                <Link key={task.id} to={`/task/${task.id}`} className="block p-5 transition-colors hover:bg-accent/60">
